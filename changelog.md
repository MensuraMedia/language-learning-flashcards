...
| 2026-08-12T02:10:00 | **Session recap could not be scrolled or left.** The panel had no max-height so it sized to its content, and the overlay centres its child — pushing the overflow off both ends where nothing scrolls. On a ten-card deck the last cards and both closing buttons were unreachable |
| 2026-08-12T02:13:00 | Fixed with three properties that must hold together: max-height on the panel, `flex: 1 1 auto; min-height: 0` on the scroll area (the easy miss — a flex item's default minimum is its content), and the actions moved outside the scroll area so leaving never requires scrolling |
| 2026-08-12T02:16:00 | Same guard applied to the other full-screen overlays — help/shortcuts and the cleared-board panel; settings already had one. All four now bounded by the window |
| 2026-08-12T02:22:00 | Recap panel widened 400px → 980px. Both tile grids are auto-fill, so width converts directly into columns; the four metrics use auto-fit and collapse onto one row |
| 2026-08-12T02:25:00 | Measured: a 10-card session now fits with no scrolling (2 columns × 5 rows); a 14-card session scrolls with the actions still pinned. Verified on text tiles and kanji glyph tiles |
| 2026-08-12T02:28:00 | Tests assert the three recap properties individually — the nesting one by parsing the template rather than counting tags — and that all four overlays are bounded. 366 passing |
| 2026-08-12T03:05:00 | **The recap's top row was covered, not clipped.** Modal overlays sat at z-index 6/30/40 against the topbar's 60, so a full-height panel slid under the chrome — the metrics row was sheared in half and scrolling could not recover it, because those pixels were painted over rather than overflowing |
| 2026-08-12T03:08:00 | Added --z-topbar and --z-modal tokens; .recap, .help and .game-done all raised above the topbar. Raising the overlay rather than padding it down means the panel gets the whole window and max-height:100% means what it says |
| 2026-08-12T03:10:00 | Settings had escaped notice only because .settings-card's 88vh kept it small — it shares the .help overlay and was equally at risk |
| 2026-08-12T03:14:00 | UI-FORMAT.md §6a is now four properties, with stacking first. Test asserts --z-modal outranks the topbar and that every overlay uses the token rather than its own number. 367 passing |
